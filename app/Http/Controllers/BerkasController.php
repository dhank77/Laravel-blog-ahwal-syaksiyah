...
class BerkasController extends Controller
{
    ...
    {
        $download = Download::where('is_public', 0)->latest()->get();
    ...
    public function delete(Download $download)
    {
        $cr = $download->delete();
        if($cr){
            return redirect(route("berkas.index"))->with('success', 'Berhasil menghapus data');
        }else{
            return redirect(route("berkas.index"))->with('error', 'Gagal menghapus data');
        }
    }
